...
const CanvasSection = class {
    constructor(hostRef) {
        ...
        this.setupCanvasClickListener = () => {
            ...
            this.gridContainerRef.addEventListener('click', (event) => {
                ...
                if (event.target === this.gridContainerRef) {
                    ...
                    stateManager.setActiveCanvas(this.canvasId);
        ...
        this.initializeDropzone = () => {
            ...
            const interactable = interact_min.interact(this.gridContainerRef);
                ...
                listeners: {
                    ...
                    drop: (event) => {
                        ...
                        if (isPaletteItem) {
                            ...
                            const widthPx = gridCalculations.gridToPixelsX(defaultWidth, this.canvasId, this.config);
                            const heightPx = gridCalculations.gridToPixelsY(defaultHeight);
    ...
    componentWillLoad() {
        // Initial load
        this.canvas = stateManager.state.canvases[this.canvasId];
        // Calculate initial height
        this.calculatedHeight = canvasHeightCalculator.calculateCanvasHeight(this.canvasId, this.config);
        // Subscribe to state changes
        stateManager.onChange('canvases', () => {
            try {
                if (this.canvasId && stateManager.state.canvases[this.canvasId]) {
                    this.canvas = stateManager.state.canvases[this.canvasId];
    ...
    disconnectedCallback() {
        // Cleanup interact.js
        if (this.gridContainerRef && this.dropzoneInitialized) {
            interact_min.interact(this.gridContainerRef).unset();
        }
    ...
    handleCanvasIdChange(newCanvasId, oldCanvasId) {
        ...
        this.canvas = stateManager.state.canvases[newCanvasId];
        // Recalculate canvas height for new canvas
        this.calculatedHeight = canvasHeightCalculator.calculateCanvasHeight(newCanvasId, this.config);
    ...
    handleConfigChange(newConfig, oldConfig) {
        ...
        this.calculatedHeight = canvasHeightCalculator.calculateCanvasHeight(this.canvasId, newConfig);
    ...
    render() {
        var _a, _b, _c;
        const showGrid = stateManager.state.showGrid;
        const verticalGridSize = gridCalculations.getGridSizeVertical(this.config);
        ...
        return (index.h("div", { key: '3dc37262a5f15cf588dc292cb61cbb67ed865cf9', class: "canvas-section", "data-canvas-id": this.canvasId }, index.h("div", { key: '9a5ad4c2ccc630070eefca751d9c5c0d6aa4490f', class: {
                'grid-container': true,
                'hide-grid': !showGrid,
                'active': this.isActive,
                'drop-target': this.isDropTarget,
            }, id: this.canvasId, "data-canvas-id": this.canvasId, style: Object.assign({ backgroundColor: this.backgroundColor || '#ffffff', backgroundSize: `2% ${verticalGridSize}px`, minHeight: `${minHeightPx}px` }, (this.calculatedHeight > 0 ? { height: `${this.calculatedHeight}px` } : {})), ref: (el) => (this.gridContainerRef = el) }, (_c = this.canvas) === null || _c === void 0 ? void 0 : _c.items.map((item) => (index.h("grid-item-wrapper", { key: item.id, item: item, renderVersion: this.renderVersion, config: this.config, componentRegistry: this.componentRegistry, onBeforeDelete: this.onBeforeDelete }))))));
    }
...
const ComponentPalette = class {
    constructor(hostRef) {
        ...
        this.initializePaletteItems = () => {
            ...
            paletteItems.forEach((element) => {
                ...
                interact_min.interact(element).draggable({
                    inertia: false,
                    autoScroll: false,
                    listeners: {
                        /**
                        ...
                        start: (event) => {
                            ...
                            event.target._originalPosition = {
                                left: paletteRect.left,
                                top: paletteRect.top,
                            };
                            event.target._dropWasValid = false; // Flag to track if drop occurred
                            ...
                            if (canvasId) {
                                ...
                                widthPx = gridCalculations.gridToPixelsX(defaultSize.width, canvasId, this.config);
                                heightPx = gridCalculations.gridToPixelsY(defaultSize.height);
                            }
                            else {
              ...
              `;
                            ...
                            event.target._defaultWidth = defaultSize.width;
                            event.target._defaultHeight = defaultSize.height;
                        },
                        /**
                         ...
                         */
                            ...
                            this.draggingItemType = null;
                            const dragClone = event.target._dragClone;
                            const dropWasValid = event.target._dropWasValid;
                            const originalPos = event.target._originalPosition;
                            if (dragClone) {
                                ...
                                delete event.target._halfWidth;
                                delete event.target._halfHeight;
                                delete event.target._dropWasValid;
                                delete event.target._originalPosition;
                            }
                        },
                    },
                });
    ...
    render() {
        ...
        if (!this.components || this.components.length === 0) {
            return (index.h("div", { class: paletteClasses }, this.showHeader && index.h("h2", null, "Components"), index.h("p", { class: "palette-empty" }, "No components available")));
        }
        return (index.h("div", { class: paletteClasses }, this.showHeader && index.h("h2", null, "Components"), this.components.map((component) => {
            // Class binding with reactive state
            const itemClasses = {
                'palette-item': true,
                'dragging-from-palette': this.draggingItemType === component.type,
            };
            return (index.h("div", { class: itemClasses, "data-component-type": component.type, key: component.type }, component.renderPaletteItem
    ...
    disconnectedCallback() {
        ...
        paletteItems.forEach((element) => {
            if (element._dragInitialized) {
                interact_min.interact(element).unset();
...
exports.canvas_section = CanvasSection;
exports.component_palette = ComponentPalette;
//# sourceMappingURL=canvas-section.component-palette.entry.cjs.js.map
