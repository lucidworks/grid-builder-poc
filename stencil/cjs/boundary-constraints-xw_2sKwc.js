...
class MoveItemCommand {
    ...
    undo() {
        ...
        let targetCanvas = stateManager.state.canvases[this.targetCanvasId];
        ...
        const sourceCanvas = stateManager.state.canvases[this.sourceCanvasId];
        if (!sourceCanvas) {
    ...
    redo() {
        ...
        const targetCanvas = stateManager.state.canvases[this.targetCanvasId];
        if (!targetCanvas) {
...
class BatchAddCommand {
    constructor(itemIds) {
        ...
        this.itemsData = itemIds.map((id) => {
            const item = Object.values(stateManager.state.canvases)
    ...
    redo() {
        ...
        const updatedCanvases = Object.assign({}, stateManager.state.canvases);
        ...
        stateManager.state.canvases = updatedCanvases;
    }
}
/**
...
class AddCanvasCommand {
    ...
    undo() {
        ...
        delete stateManager.state.canvases[this.canvasId];
        ...
        eventManager.eventManager.emit('canvasRemoved', { canvasId: this.canvasId });
    }
    redo() {
        // Add canvas to library state (minimal - just item placement management)
        stateManager.state.canvases[this.canvasId] = {
            zIndexCounter: 1,
            items: [],
        };
        // Trigger state change for reactivity
        stateManager.state.canvases = Object.assign({}, stateManager.state.canvases);
        // Emit event so host app can sync its metadata
        eventManager.eventManager.emit('canvasAdded', { canvasId: this.canvasId });
    }
}
/**
 * RemoveCanvasCommand
...
class RemoveCanvasCommand {
    ...
    undo() {
        ...
        if (this.canvasSnapshot) {
            stateManager.state.canvases[this.canvasId] = JSON.parse(JSON.stringify(this.canvasSnapshot));
            // Trigger state change for reactivity
            stateManager.state.canvases = Object.assign({}, stateManager.state.canvases);
            // Emit event so host app can sync its metadata
            eventManager.eventManager.emit('canvasAdded', { canvasId: this.canvasId });
 ...
 */
const { state: undoRedoState } = stateManager.createStore({
    canUndo: false,
    canRedo: false,
});
/**
...
exports.AddCanvasCommand = AddCanvasCommand;
exports.BatchAddCommand = BatchAddCommand;
exports.BatchDeleteCommand = BatchDeleteCommand;
exports.BatchUpdateConfigCommand = BatchUpdateConfigCommand;
exports.CANVAS_WIDTH_UNITS = CANVAS_WIDTH_UNITS;
exports.MoveItemCommand = MoveItemCommand;
exports.RemoveCanvasCommand = RemoveCanvasCommand;
exports.applyBoundaryConstraints = applyBoundaryConstraints;
exports.constrainPositionToCanvas = constrainPositionToCanvas;
exports.createDebugLogger = createDebugLogger;
exports.pushCommand = pushCommand;
exports.undoRedo = undoRedo;
exports.undoRedoState = undoRedoState;
exports.virtualRenderer = virtualRenderer;
//# sourceMappingURL=boundary-constraints-xw_2sKwc.js.map
