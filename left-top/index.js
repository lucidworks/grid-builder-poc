    ...
    .draggable({
      ...
      listeners: {
        ...
        move(event) {
          ...
          // Apply drag delta directly - interact.js already throttles
          ...
          // oldCanvasId already declared above at line 1207
    ...
    .resizable({
      ...
      listeners: {
        start(event) {
          ...
          startRect.left = parseFloat(event.target.style.left) || 0;
        ...
        move(event) {
          ...
          const newX = parseFloat(event.target.style.left);
          const newY = parseFloat(event.target.style.top);
...
function nudgeItem(id, canvasId, dx, dy) {
  ...
  if (item) {
    ...
    // Update DOM (convert to pixels)
    const element = document.getElementById(id);
    if (element) {
      element.style.left = gridToPixelsX(layout.x, canvasId) + 'px';
      element.style.top = gridToPixelsY(layout.y) + 'px';
    }
