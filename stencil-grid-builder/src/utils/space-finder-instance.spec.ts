 ...
 * - Centered positioning for empty canvas - no state needed
...
describe("space-finder (instance-based)", () => {
  ...
  describe("checkCollision", () => {
    // checkCollision is pure function, no state needed - same tests as original
  ...
  describe("getCenteredPosition", () => {
    // getCenteredPosition is pure function, no state needed - same tests as original
  ...
  describe("getBottomPosition", () => {
    it("should return (0, 0) for empty canvas", () => {
      ...
      const result = getBottomPosition("canvas1", testState);
    ...
    it("should return (0, 0) for non-existent canvas", () => {
      // Pass testState explicitly (instance-based)
      const result = getBottomPosition("non-existent", testState);
  ...
  describe("findFreeSpace", () => {
    ...
    it("should return null for non-existent canvas", () => {
      // Pass testState explicitly (instance-based)
      const result = findFreeSpace("non-existent", 10, 6, testState);
    ...
    it("should find free space via grid scan when top-left is occupied", () => {
      ...
      if (result) {
        ...
        const hasCollision = checkCollision(
          testPos,
          testState.canvases.canvas1.items[0].layouts.desktop,
        );
    ...
    it("should find space between existing items", () => {
      ...
      // Pass testState explicitly (instance-based)
      const result = findFreeSpace("canvas1", 5, 3, testState);

      // Should find space (e.g., between items or to the right)
      expect(result).not.toBeNull();
      if (result) {
        const testPos = { ...result, width: 5, height: 3 };
        testState.canvases.canvas1.items.forEach((item) => {
    ...
    it("should place at bottom when no free space found in grid scan", () => {
      ...
      testState.canvases.canvas1.items = items;
    ...
    it("should respect component width constraints", () => {
      ...
      // Pass testState explicitly (instance-based)
      const result = findFreeSpace("canvas1", 25, 6, testState);
    ...
    it("should handle full-width components", () => {
      // Pass testState explicitly (instance-based)
      const result = findFreeSpace("canvas1", 50, 6, testState);
    ...
    it("should handle small components efficiently", () => {
      ...
      // Small component (2×2)
      // Pass testState explicitly (instance-based)
      const result = findFreeSpace("canvas1", 2, 2, testState);
