...
describe("undo-redo-commands", () => {
  ...
  describe("MoveItemCommand", () => {
    ...
    it("should undo item resize by restoring original size", () => {
      ...
      command.undo();
      expect(gridState.canvases.canvas1.items[0].layouts.desktop.width).toBe(
        20,
      );
      expect(gridState.canvases.canvas1.items[0].layouts.desktop.height).toBe(
        15,
      );
  ...
  describe("Complex Scenarios", () => {
    it("should handle sequence of add, delete, move commands", () => {
      ...
      const moveCmd = new MoveItemCommand(
        "item-1",
        "canvas1",
        "canvas2",
        { x: 0, y: 0 },
  ...
  describe("Batch Commands", () => {
    ...
    describe("BatchUpdateConfigCommand", () => {
      ...
      it("should handle multiple config updates", () => {
        ...
        expect(gridState.canvases.canvas1.items[1].config.content).toBe(
          "New 2",
        );
      ...
      it("should handle updates across multiple canvases", () => {
        const items = [
          {
            ...
            type: "header",
            name: "Header",
            config: { text: "Canvas 1" },
          },
          {
            canvasId: "canvas2",
            type: "text",
            name: "Text",
            config: { text: "Canvas 2" },
          },
        ];
        const itemIds = addItemsBatch(items);
        ...
        expect(gridState.canvases.canvas2.items[0].config.text).toBe(
          "Updated 2",
        );
