...
| `eventManagerInstance`    | --                 | Event manager service instance (Phase 3: Instance-based architecture)  **Optional prop**: Service instance for event emission **Default**: grid-item-wrapper falls back to singleton if not provided **Source**: grid-builder → canvas-section → grid-item-wrapper  **Purpose**: Support multiple grid-builder instances with isolated services  **Migration strategy**: - Phase 3: Add as optional prop (this phase) - Phase 4: Remove singleton fallback and make required                                                                                                                                               | `EventManager`                                         | `undefined` |
...
| `onStateChange`           | --                 | State change subscription function (Phase 3: Instance-based architecture)  **Optional prop**: Function to subscribe to instance state changes **Default**: Falls back to singleton onChange if not provided **Source**: grid-builder (this.stateManager.onChange)  **Purpose**: Subscribe to instance-specific state changes for reactivity  **Usage**: ```typescript onStateChange={(key, callback) => this.stateManager.onChange(key, callback)} ```                                                                                                                                                                     | `(key: string, callback: Function) => void`            | `undefined` |
...
| `virtualRendererInstance` | --                 | Virtual renderer service instance (Phase 3: Instance-based architecture)  **Optional prop**: Service instance for lazy loading **Default**: grid-item-wrapper falls back to singleton if not provided **Source**: grid-builder → canvas-section → grid-item-wrapper  **Purpose**: Support multiple grid-builder instances with isolated services  **Migration strategy**: - Phase 3: Add as optional prop (this phase) - Phase 4: Remove singleton fallback and make required                                                                                                                                              | `VirtualRendererService`                               | `undefined` |
