import { g as gridState, c as updateItem, e as addItemToCanvas, d as deleteItemsBatch, u as updateItemsBatch, f as setItemZIndex } from './state-manager-BtBFePO6.js';
...
class ChangeZIndexCommand {
    constructor(changes, eventManager) {
    ...
    undo() {
        ...
        this.changes.forEach((change) => {
            const result = setItemZIndex(change.canvasId, change.itemId, change.oldZIndex);
            if (result) {
                debug.log(`Undo z-index change: ${change.itemId} from ${result.newZIndex} to ${result.oldZIndex}`);
            }
        });
    ...
    redo() {
        ...
        this.changes.forEach((change) => {
            const result = setItemZIndex(change.canvasId, change.itemId, change.newZIndex);
            if (result) {
                debug.log(`Redo z-index change: ${change.itemId} from ${result.oldZIndex} to ${result.newZIndex}`);
            }
        });
        // Emit single event (batch or individual based on change count)
        if (this.changes.length === 1) {
...
export { AddCanvasCommand as A, BatchUpdateConfigCommand as B, CANVAS_WIDTH_UNITS as C, MoveItemCommand as M, RemoveCanvasCommand as R, applyBoundaryConstraints as a, BatchDeleteCommand as b, constrainPositionToCanvas as c, BatchAddCommand as d };
//# sourceMappingURL=boundary-constraints-Bp5RmeLZ.js.map
