 ...
 */
function getBottomPosition(canvasId, state) {
    ...
    const bottomY = Math.max(...canvas.items.map((item) => item.layouts.desktop.y + item.layouts.desktop.height));
 ...
 */
function findFreeSpace(canvasId, width, height, state) {
    const stateToUse = state || gridState;
    const canvas = stateToUse.canvases[canvasId];
...
export { CANVAS_WIDTH_UNITS, checkCollision, findFreeSpace, getBottomPosition, getCenteredPosition };
//# sourceMappingURL=space-finder-CZfpZ5hA.js.map
